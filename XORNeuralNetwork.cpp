...
class XORModel {
public:
    XORModel(int input_size, int hidden_size, int output_size) {
        // Initialize weights and biases randomly with a fixed seed
        hidden_weights.resize(hidden_size, std::vector<double>(input_size));
        hidden_biases.resize(hidden_size, 0.0);
        output_weights.resize(output_size, std::vector<double>(hidden_size));
        output_biases.resize(output_size, 0.0);
    ...
    double compute_accuracy() {
        ...
        return static_cast<double>(correct_predictions) / X.size() * 100.0; // Return accuracy as a percentage
    }
